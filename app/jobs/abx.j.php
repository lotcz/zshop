<?php
	...
	if (file_exists($xml_path)) {
		...
		foreach ($xml->categories->category as $category) {
			$cat_total += 1;
			$zCategory = new CategoryModel($db);
			...
			if (isset($category->parentid) && $category->parentid > 0) {
				...
				if ($parent->is_loaded) {
					$zCategory->data['category_parent_id'] = $parent->ival('category_id');
				}
			}
		...
		$categories_tree = CategoryModel::getCategoryTree($db);
		...
		foreach ($xml->products->product as $product) {
			...
			if (strpos($product_name, ' var.') > 0) {
				...
				if (strlen($variant_name) > 0) {
					$zVariant = new ProductVariantModel($db);
